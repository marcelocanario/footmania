...
async function setupClub(app: FastifyInstance, username: string, seed = 4242) {
  ...
  const join = await app.inject({
    ...
    url: "/api/mp/join",
    headers: { cookie },
    payload: { clubName: `${username} FC`, country: "BRA", timezone: "America/Sao_Paulo", stadiumName: `${username} Stadium`, coachName: `${username} Coach`, preferredHours: Array.from({ length: 16 }, (_, i) => i) },
  });
