...
calibrationDescribe("accelerated career simulation (spec §60-§61)", () => {
  ...
  it("naturally produces the full spectrum of careers (spec §61)", () => {
    ...
    const poor = samples.filter((s) => s.start <= 45);
    const average = samples.filter((s) => s.start > 45 && s.start < 60);
    const great = samples.filter((s) => s.start >= 60);
...
describe("academy lifecycle & reroll exploit tests (spec §66)", () => {
  it("fixed seasonal intake: releasing youth before intake does not increase the quota", () => {
    ...
    // Release all youth; the seasonal intake still generates at most 2.
    ...
    expect(intake.length).toBeLessThanOrEqual(2);
  });

  it("empty academy still receives exactly 2 (not a refill to 8 or 12)", () => {
...
describe("club creation idempotency (spec §46)", () => {
  it("re-generating a club roster does not duplicate players", () => {
    ...
    expect(seniorsAfterFirst).toHaveLength(28);
  ...
  it("guarded club-roster generation records a creation event and is a no-op on retry", () => {
    ...
    expect(first.seniors).toHaveLength(28);
    expect(first.youth).toHaveLength(8);
    ...
    expect(second.seniors).toHaveLength(28);
    expect(second.youth).toHaveLength(8);
