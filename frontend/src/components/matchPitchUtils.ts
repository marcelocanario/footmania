...
/**
 * On-pitch point for every player of one team, from the backend-provided slot
 * coordinates (§15.4). Each LivePlayer's `slotIndex` maps directly to
 * `slots[slotIndex]`; x is mirrored for the away side. The backend formation
 * catalog is the only geometry authority, so there is no local layout table to
 * fall back to — a player without a valid slot simply is not drawn.
 */
export function teamPitchPointsFromSlots(
  players: LivePlayer[],
  side: PitchSide,
  slots: Array<{ x: number; y: number }> | undefined,
): Map<number, PitchPoint> | null {
  if (!slots || slots.length === 0) return null;
  const points = new Map<number, PitchPoint>();
  for (const p of players) {
    const idx = (p as unknown as { slotIndex?: number | null }).slotIndex;
    if (idx === null || idx === undefined || idx < 0 || idx >= slots.length) continue;
    const s = slots[idx];
    const x = side === "home" ? s.x : 100 - s.x;
    points.set(p.id, { x, y: s.y });
  }
...
export function placeLiveBall(ball: LiveBall, players: LivePlayer[], points: Map<number, PitchPoint>): BallPlacement {
  ...
  const candidates = players
    .filter((p) => (p as unknown as { deployedRole?: string | null }).deployedRole !== "GK" || ball.startType === "GOAL_KICK")
...
export function turnoverIntent(
  ...
  const candidates = players
    .filter((p) => (p as unknown as { deployedRole?: string | null }).deployedRole !== "GK")
    .map((p) => ({ p, pt: points.get(p.id) }))
    .filter((c): c is { p: LivePlayer; pt: PitchPoint } => !!c.pt)
    .sort((a, b) => {
